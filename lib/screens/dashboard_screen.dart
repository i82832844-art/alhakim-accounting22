  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.all(18), children: [
        ...
        const SizedBox(height: 12),
        Row(children: [Expanded(child: _quick('مسح باركود', Icons.qr_code_scanner, () async { await Navigator.push(context, MaterialPageRoute(builder: (_) => const ScanScreen())); _load(); })), const SizedBox(width: 12), Expanded(child: _quick('إضافة منتج', Icons.add_box_outlined, () async { await Navigator.push(context, MaterialPageRoute(builder: (_) => const ProductFormScreen())); _load(); }))]),
        const SizedBox(height: 12),
        ...
        Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [Text('الحكيم للمحاسبة — الإصدار 0.2.0', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)), SizedBox(height: 6), Text('قاعدة بيانات محلية، إدارة المنتجات، باركود، مبيعات وفواتير، خصم تلقائي من المخزون، ومؤشرات مبيعات اليوم. المرحلة التالية: المشتريات والمصروفات والتقارير والنسخ الاحتياطي والتعرف الذكي بالصورة.')]))),
      ])),
    );
  }
